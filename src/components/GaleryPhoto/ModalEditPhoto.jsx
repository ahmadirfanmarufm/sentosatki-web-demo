...
const ModalEditPhoto = ({ isOpen, closeModal, toast, selectedPhotoId, refreshPhoto }) => {
    ...
    useEffect(() => {
        axios.get(`${apiDatabaseUrl}/gallery-photos/${selectedPhotoId}`)
        .then((response) => { 
            const data = response.data;
            setTitle(data.title);
            setCategory(data.category);
            setDescription(data.description);
            setImagePreview(`${apiDatabaseUrl}/uploads/${data.image_url}`);
        })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`${apiDatabaseUrl}/edit/gallery-photos/photos/${selectedPhotoId}`, formData, {
                headers: {
