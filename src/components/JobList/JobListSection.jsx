  ...
  useEffect(() => {
    ...
    const filtered = jobs.filter((job) => {
      ...
      const matchesCategorySector =
        categorySector === 'All' ||
        job.sector.toLowerCase() === categorySector.toLowerCase();
      const matchesLocation =
        location === 'All' ||
        job.location.toLowerCase().includes(location.toLowerCase());
      const matchesSalary =
        !salaryFilter || isSalaryInRange(job.salary, salaryFilter);
                    ...
                    <div className="flex items-start space-x-4 w-full md:w-3/4 text-left">
                      ...
                      <div>
                        ...
                        <div className="flex flex-col lg:flex-row lg:space-x-4 md:mt-5">
                          ...
                          {exchangeRates &&
                            exchangeRates[
                              currencyMap[job.salary.split(' ')[0]]
                            ] && (
                              <p className="grid grid-cols-[auto,1fr] gap-1 items-center mr-3 items-center text-gray-600">
                                ...
                                {formatCurrency(
                                  parseFloat(
                                    job.salary.split(' ')[1].replace(/\D/g, ''),
                                  ) /
                                    exchangeRates[
                                      currencyMap[job.salary.split(' ')[0]]
                                    ],
