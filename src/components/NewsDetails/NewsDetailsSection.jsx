  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        ...
        const relatedResponse = await axios.get(`${apiDatabaseUrl}/berita`);
        setRelatedArticles(
          relatedResponse.data.filter(
            (art) => art.category === response.data.category && art.id !== id,
          ),
        );

        const latestResponse = await axios.get(`${apiDatabaseUrl}/berita`);
        setLatestArticles(latestResponse.data.filter((news) => news.id !== id));
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-md overflow-hidden my-20 p-4">
      <img
        ...
        src={`${apiDatabaseUrl}/uploads/${article.image}`}
        ...
        <div className="flex-1 md:pr-8 text-left">
          ...
          {user && user.jabatan.includes('Writter') && (
            <div className="flex flex-col space-y-2">
              <button
                className="bg-primary-secondary-800 hover:bg-primary-secondary-900 text-white px-4 py-2 rounded-md"
              ...
              {relatedArticles.slice(0, 3).map((post) => (
                ...
                >
                  <div className="flex items-center gap-x-4">
                    <img
                      src={`${apiDatabaseUrl}/uploads/${post.image}`}
