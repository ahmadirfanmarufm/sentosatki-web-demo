...
const Registration = () => {
    ...
    const [modalAnimation, setModalAnimation] = useState('animate-slide-in-down');
    ...
    const openModal = () => {
        setModalAnimation('animate-slide-in-down');
        setModalOpen(true);
        document.body.classList.add('modal-open');
    };

    const closeModal = () => {
        setModalAnimation('animate-slide-out-up');
        setTimeout(() => {
            setModalOpen(false);
    ...
    return (
        <div className="App">
            <div>
                ...
                {/* Form Registrasi */}
                <RegistrationForm isCheckboxChecked={isCheckboxChecked} handleCheckboxChange={handleCheckboxChange} />

                {/* Modal Syarat dan Ketentuan */}
                <Dialog open={modalOpen} onClose={closeModal} className="fixed inset-0 z-50 flex items-start justify-center bg-gray-800 bg-opacity-50 overflow-y-auto">
                    <DialogBackdrop className="fixed inset-0 bg-black opacity-50" />
                    <DialogPanel className={`relative w-full max-w-2xl px-4 mt-4 bg-white rounded-lg shadow p-8 ${modalAnimation}`}>
                        {/* Header */}
                        ...
                        <div className="p-6 space-y-6 border-t border-gray-300">
                            <ol className="list-decimal pl-6 space-y-4 text-gray-700 text-base leading-relaxed">
                                <li>Telah mengisi data ini dengan benar untuk kepentingan Registrasi Sebagai Calon Tenaga Kerja Luar Negeri (Calon Pekerja Migran Indonesia).</li>
                                <li>Menyetujui data saya dapat di Akses oleh Agency Resmi (P3MI) Perusahaan Penempatan Pekerja Migran Indonesia dan Agency di Luar Negeri (Negara Penempatan).</li>
                ...
                {isModalOpen && <LoginModal closeModal={closeModalLogin} isOpen={isModalOpen} />}
            </div>
        </div>
    );
}
